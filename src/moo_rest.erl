...
method_allowed(Req, #state{method=Method, handler=Module} = State) ->
	case erlang:function_exported(Module, Method, method_arity(Method)) of
		true ->
...
process_request(Method, Req, #state{handler=Handler, handler_state=HandlerState} = State) when Method =:= get; Method =:= head ->
...
process_request(Method, Req, State) when
	...
	case cowboy_req:parse_header(<<"content-type">>, Req) of
		{ok, ContentType, Req2} ->
			accept_resource(ContentType, Method, Req2, State);
		{error, badarg} ->
...
accept_resource(ContentType, Method, Req, #state{handler=Handler, handler_state=HandlerState} = State) ->
	try Handler:Method(ContentType, Req, HandlerState) of
		{ok, Req2, NewHandlerState} ->
			render(render_result, Req2, State#state{handler_state=NewHandlerState});
...
method_arity(Method) ->
	case (Method =:= put) or (Method =:= post) or (Method =:= patch) of
		true -> 3;
		false -> 2
	end.
...
stop(Req, #state{env=Env}) -> {ok, Req, Env}.

error_terminate(Reason, Method, Req, #state{handler=Module, handler_state=State, env=Env}) ->
